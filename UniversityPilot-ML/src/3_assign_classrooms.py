...
# 3. Odfiltruj wykłady (nie przypisujemy sal wykładom)
schedule_df = schedule_df[schedule_df['Rodzaj'] != 'Wykład']

# 4. Merge z preferencjami sal tylko dla istniejących przedmiotów i rodzajów
# Przed mergem — usuń duplikaty
...
# 6. Wyciągnięcie unikalnych sal z TopN
...
# 8. Preferencje jako koszty
penalty_terms = []
for idx, row in df.iterrows():
    penalties = [5] * len(all_classrooms)  # Domyślnie duża kara
    for col in top_n_columns:
        room = row[col]
        if pd.notna(room) and room in classroom_to_id:
            penalties[classroom_to_id[room]] = 0  # Zero kara dla preferowanych
...
# 9. Ograniczenia kolizji (ta sama sala w tym samym czasie)
...
for date, group in grouped:
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if not (end_i <= start_j or end_j <= start_i):  # Jeśli się nachodzą
